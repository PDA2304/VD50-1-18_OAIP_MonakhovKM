...
namespace VD50_1_18_OAIP_MonakhovKM_PR3
{
    class User : config
    {
        ...
        public bool Authentication(string LoginBox, string PasswordBox)
        {
            ...
            var data_table = data_base.Select("SELECT `id_user`, `id_group`, `user_login`, `user_email`, `user_password`, `last_name`, `first_name`, `mid_name` FROM `users` JOIN `employee` WHERE(`users`.`user_login` = '" + LoginBox + "'OR `users`.`user_email` = '" + LoginBox + "') AND(BINARY `users`.`user_password` = '" + Hash(PasswordBox) + "') AND(`users`.`id_user` = `employee`.`id_employee`)");
            if (!Status)
            {
                ...
                MidName = data_table.Rows[0]["mid_name"].ToString(); CacheAuth();
            }
            return Status;
        }
        ...
        public int CheckAuth()
        {
            ...
            {
                Database data_base = new Database();
                string sql = "SELECT `tokens`.`id_user`, `token`, `last_name`, `first_name`, `mid_name` FROM `tokens` JOIN `users`, `employee` WHERE(`tokens`.`id_user` ='" + localSettings.Values["id_user"] + "') AND(`token` = '" + localSettings.Values["token"] + "')AND(`deactivated` = 0) AND(`tokens`.`id_user` = `users`.`id_user` AND `users`.`id_employee` =`employee`.`id_employee`)";
