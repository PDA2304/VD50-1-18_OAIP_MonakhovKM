...
namespace VD50_1_18_OAIP_MonakhovKM_PR3.Assets
{
    class config
    {
        ...
        public MySqlConnection connection = new MySqlConnection("server=127.0.0.1; user=root; password=; database=movedb;   port=3306");
        ...
        public string salt = "ТРЕБУЕТСЯ_ЗАДАТЬ_САМОСТОЯТЕЛЬНО";
        ...
        public string GenerateToken(int user)
        {
            ...
            string token = new string(Enumerable.Repeat(chars, 64).Select(s
            => s[random.Next(s.Length)]).ToArray());
            string sql = "INSERT INTO `tokens` (`id_user`, `token`) VALUES ("
            + user + ", '" + token + "')";
            data_base.Insert(sql);
            // Вызов метода занесения действий в журнал с передачей идентификатора пользователя и первого типа действий в соответствии с базой данных(авторизация)
            Log(user, 1);
